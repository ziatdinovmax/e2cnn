...
def offset_iterator(
    base_frequency, N, maximum_offset=None, maximum_frequency=None, non_negative = False
):
    if N < 0:
        # assert maximum_offset == 0
        if (
            maximum_frequency is not None
            and math.fabs(base_frequency) <= maximum_frequency
        ):
            yield 0
    else:
        ...
        if non_negative:
            ...
            minimum_frequency = (
                -maximum_frequency if maximum_frequency is not None else None
            )
...
def psi(theta, k = 1, gamma = 0.0, out = None):
...
def psichi(theta, s, k = 1, gamma = 0.0, out = None):
...
if __name__ == "__main__":
    ...
    gamma = np.array([0.0, 0.0, np.pi / 2])
    k = np.array([1, 2, 4])
    s = np.array([1, 0, 0])
