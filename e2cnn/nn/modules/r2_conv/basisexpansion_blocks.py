...
class BlocksBasisExpansion(BasisExpansion):
    def __init__(
        ...
        **kwargs,
        ...
        for i_repr in in_type._unique_representations:
            for o_repr in out_type._unique_representations:
                ...
                try:
                    ...
                    basis = space.build_kernel_basis(
                        i_repr, o_repr, sigma=sigma, rings=rings, **kwargs
                    )

                    block_expansion = block_basisexpansion(
                        basis, points, basis_filter, recompute=recompute
                    )
        ...
        self._n_pairs = len(in_type._unique_representations) * len(
            out_type._unique_representations
        )
        ...
        for io_pair in self._representations_pairs:
            ...
            if self._contiguous[io_pair]:
                ...
                out_indices, in_indices = torch.meshgrid(
                    [_out_indices[io_pair[1]], _in_indices[io_pair[0]]]
                )
            ...
            for o in out_block_counts[reprs_names[1]]:
                ...
                for i in in_block_counts[reprs_names[0]]:
                    ...
                    for attr in block_expansion.get_basis_info():
                        ...
                        # add names and indices of the input and output fields
                        id = "({}-{},{}-{})".format(
                            reprs_names[0], i, reprs_names[1], o
                        )
                        # add the original id in the block submodule
                        id += "_" + attr["id"]
    ...
    def _expand_block(self, weights, io_pair):
        ...
        _filter = _filter.view(
            self._out_count[io_pair[1]], self._in_count[io_pair[0]], o, i, self.S,
        )
    ...
    def forward(self, weights):
        ...
        if self._n_pairs == 1:
            ...
            _filter = self._expand_block(weights, io_pair).reshape(
                out_indices[2], in_indices[2], self.S
            )
            ...
            _filter = torch.zeros(
                self._output_size, self._input_size, self.S, device=weights.device
            )
            ...
            for io_pair in self._representations_pairs:
                ...
                if self._contiguous[io_pair]:
                    ...
                    _filter[out_indices, in_indices, :,] = expanded.reshape(-1, self.S)

        # return the new filter
        return _filter


def _retrieve_indices(type):
    fiber_position = 0
