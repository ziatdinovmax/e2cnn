...
class R2Conv(EquivariantModule):
    def __init__(
        ...
        if isinstance(padding, tuple) and len(padding) == 2:
            ...
            raise ValueError(
                "padding needs to be either an integer or a tuple containing two"
                " integers but {} found".format(padding)
            )
        ...
        self.register_buffer(
            "filter", torch.zeros(out_type.size, in_type.size, kernel_size, kernel_size)
        )
    ...
    def expand_parameters(self):
        ...
        _filter = _filter.reshape(
            _filter.shape[0], _filter.shape[1], self.kernel_size, self.kernel_size
        )
    ...
    def evaluate_output_shape(self, input_shape):
        ...
        ho = math.floor(
            (hi + 2 * self.padding - self.dilation * (self.kernel_size - 1) - 1)
            / self.stride
            + 1
        )
        wo = math.floor(
            (wi + 2 * self.padding - self.dilation * (self.kernel_size - 1) - 1)
            / self.stride
            + 1
        )
    ...
    def check_equivariance(
        self, atol = 0.1, rtol = 0.1, assertion = True, verbose = True
    ):
        ...
        x = mpimg.imread("../group/testimage.jpeg").transpose((2, 0, 1))[
            np.newaxis, 0:c, :, :
        ]
...
def bandlimiting_filter(frequency_cutoff):
    r"""
    ...
    def bl_filter(attributes):
        return math.fabs(attributes["frequency"]) <= frequency_cutoff(
            attributes["radius"]
        )
...
def get_grid_coords(kernel_size, dilation = 1):
