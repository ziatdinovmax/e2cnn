...
class R2Upsampling(EquivariantModule):
    def __init__(self, in_type, scale_factor, mode = "bilinear", align_corners = False):
        r"""
        ...
        if mode not in ["nearest", "bilinear"]:
            raise ValueError(
                f"Error Upsampling mode {mode} not recognized! Mode should be `nearest`"
                " or `bilinear`."
            )
    ...
    def forward(self, input):
        ...
        if self._align_corners is None:
            output = interpolate(
                input.tensor, scale_factor=self._scale_factor, mode=self._mode
            )
        else:
            output = interpolate(
                input.tensor,
    ...
    def check_equivariance(self, atol = 0.1, rtol = 0.1):
        ...
        for el in self.space.testing_elements:
            ...
            center_mask = (
                center_mask[0, :, :] ** 2 + center_mask[1, :, :] ** 2 < (h * 0.4) ** 2
            )
