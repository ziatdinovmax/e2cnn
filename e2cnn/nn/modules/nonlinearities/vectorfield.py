...
class VectorFieldNonLinearity(EquivariantModule):
    def __init__(self, in_type, **kwargs):
        ...
        for r in in_type.representations:
            assert "vectorfield" in r.supported_nonlinearities, (
                'Error! Representation "{}" does not support "vector-field"'
                " non-linearity".format(r.name)
            )

            assert (
                r.name == "regular" and r.size == in_type.gspace.fibergroup.order()
            ), r.name
    ...
    def forward(self, input):
        ...
        output = torch.empty(
            b, self.out_type.size, h, w, dtype=torch.float, device=input.tensor.device
        )
