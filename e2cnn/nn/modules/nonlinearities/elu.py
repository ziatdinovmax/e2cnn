...
class ELU(EquivariantModule):
    def __init__(self, in_type, alpha = 1.0, inplace = False):
        r"""
        ...
        for r in in_type.representations:
            assert "pointwise" in r.supported_nonlinearities, (
                'Error! Representation "{}" does not support "pointwise" non-linearity'
                .format(r.name)
            )
    ...
    def forward(self, input):
        ...
        return GeometricTensor(
            F.elu(input.tensor, alpha=self.alpha, inplace=self._inplace), self.out_type
        )
