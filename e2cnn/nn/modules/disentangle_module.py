...
class DisentangleModule(EquivariantModule):
    def __init__(self, in_type):
        ...
        for r in self.in_type._unique_representations:
            ...
            self.register_buffer(
                "change_of_basis_{}".format(r.name), _change_of_basis_matrices[r.name]
            )
        ...
        for (
            repr_name,
            (contiguous, fields_indices, fiber_indices),
    ...
    def forward(self, input):
        ...
        for repr_name in self._order:
            ...
            input_fields = input_fields.view(
                b, self._nfields[repr_name], self._sizes[repr_name], w, h
            )
            ...
            transformed_fields = torch.einsum(
                "oi,bciwh->bcowh", (cob, input_fields)
            ).reshape(b, -1, w, h)
    ...
    def check_equivariance(self, atol = 1e-7, rtol = 1e-5):
