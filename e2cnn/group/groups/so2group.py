...
class SO2(Group):
    ...
    def testing_elements(self):
        ...
        """
        N = 4 * 13
        return iter([i * 2.0 * np.pi / N for i in range(N)])
    ...
    def subgroup(self, id):
        ...
        if id not in self._subgroups:
            ...
            child_mapping = (
                ...
                if not utils.cycle_isclose(e, 0.0, 2.0 * np.pi / order)
                else int(round(e * order / (2.0 * np.pi)))
            )
    ...
    def irrep(self, k):
        ...
        if name not in self.irreps:
            ...
            if k == 0:
                ...
                character = lambda element, k=k: np.cos(k * element) + np.cos(
                    k * element
                )
