...
class CyclicGroup(Group):
    ...
    def subgroup(self, id):
        ...
        assert self.order() % order == 0, (
            "Error! The subgroups of a cyclic group have an order that divides the"
            " order of the supergroup. %d does not divide %d "
            % (order, self.order())
        )
        ...
        if id not in self._subgroups:
            ...
            child_mapping = (
                lambda e, ratio=ratio: None if e % ratio != 0 else int(e // ratio)
            )
    ...
    def irrep(self, k):
        ...
        if name not in self.irreps:
            ...
            if k == 0:
                ...
                irrep = lambda element, k=k, base_angle=base_angle: utils.psi(
                    element * base_angle, k=k
                )
