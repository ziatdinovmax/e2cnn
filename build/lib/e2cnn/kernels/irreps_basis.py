...
class R2DiscreteRotationsSolution(IrrepBasis):
    ...
    def __hash__(self):
        return (
            ...
            + hash(self.out_irrep)
            + hash(self.mu.tostring())
            + hash(self.gamma.tostring())
        )


class R2ContinuousRotationsSolution(IrrepBasis):
    def __init__(
        self, group, in_irrep, out_irrep,
    ):
        ...
        dim = self.gamma.shape[0]
        super(R2ContinuousRotationsSolution, self).__init__(
            group, in_irrep, out_irrep, dim
        )
...
class R2FlipsDiscreteRotationsSolution(IrrepBasis):
    def __init__(
        ...
        if in_irrep.size == 2 and out_irrep.size == 2:
            ...
            assert (self.m == 0 or (self.m == self.N // 2 and self.N % 2 == 0)) and (
                self.fi == 1
            )
            # n > 0, m = 0 or N/2
            ...
            assert (
                self.n == 0 or (self.n == self.N // 2 and self.N % 2 == 0)
            ) and self.fo == 1
            # m > 0, n = 0 or N/2
        ...
        dim = self.mu.shape[0]
        super(R2FlipsDiscreteRotationsSolution, self).__init__(
            group, in_irrep, out_irrep, dim
        )
...
class R2FlipsContinuousRotationsSolution(IrrepBasis):
    def __init__(
        self, group, in_irrep, out_irrep, axis = 0.0,
    ):
        ...
        dim = self.mu.shape[0]
        super(R2FlipsContinuousRotationsSolution, self).__init__(
            group, in_irrep, out_irrep, dim
        )
...
if __name__ == "__main__":
    ...
    points = np.array([[0.0, 0.0], [0.0, 1.0], [1.0, 1.0], [-1, 0.0]]).T
