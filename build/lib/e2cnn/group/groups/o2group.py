...
class O2(Group):
    def __init__(self, maximum_frequency):
        r"""
    ...
    def testing_elements(self):
        ...
        N = 4 * 13
        return iter(
            [(0, i * 2.0 * np.pi / N) for i in range(N)]
            + [(1, i * 2.0 * np.pi / N) for i in range(N)]
        )
    ...
    def __eq__(self, other):
        if not isinstance(other, O2):
            ...
            return (
                ...
                and self._maximum_frequency == other._maximum_frequency
            )
    ...
    def subgroup(self, id):
        ...
        if id not in self._subgroups:
            if id[0] is not None and id[1] == -1:
                ...
                child_mapping = (
                    lambda e, axis=axis: None
                    if not utils.cycle_isclose(e[1], axis * e[0], 2 * np.pi)
                    else e[0]
                )
                ...
                parent_mapping = lambda e, order=order: (0, e * 2.0 * np.pi / order)
                # child_mapping = lambda e, order=order: None if (e[0] != 0 or not divides(2.*np.pi/order, e[1])) else \
                #                                        int(round(e[1] * order / (2.*np.pi)))
                child_mapping = (
                    lambda e, order=order: None
                    if (
                        e[0] != 0
                        or not utils.cycle_isclose(e[1], 0.0, 2.0 * np.pi / order)
                    )
                    else int(round(e[1] * order / (2.0 * np.pi)))
                )
                ...
                parent_mapping = lambda e, order=order, axis=axis: (
                    e[0],
                    e[1] * 2.0 * np.pi / order + e[0] * axis,
                )
                child_mapping = (
                    lambda e, order=order, axis=axis: None
                    if not utils.cycle_isclose(
                        e[1] - e[0] * axis, 0.0, 2.0 * np.pi / order
                    )
                    else (
                        e[0],
                        int(round((e[1] - e[0] * axis) * order / (2.0 * np.pi))),
                    )
                )
            else:
    ...
    def _restrict_irrep(self, irrep, id):
        r"""
        ...
        if name not in self.irreps:
            if j == 0:
                if k == 0:
                    ...
                    self.irreps[name] = IrreducibleRepresentation(
                        self,
                        name,
                        irrep,
                        1,
                    ...
                    raise ValueError(
                        f"Error! Flip frequency {j} and rotational frequency {k} don't"
                        f" correspond to any irrep of the group {self.name}!"
                    )
                ...
                character = (
                    lambda element, k=k: 0 if element[0] else 2 * np.cos(k * element[1])
                )
        ...
        if subgroup_id == (None, -1):
            ...
            for irr, m in multiplicities:
                ...
                if m > 0:
                    ...
                    n_repetitions = len(
                        [name for name in restricted_irr.irreps if name == repr.name]
                    )
                    assert repr.size * n_repetitions >= m, (
                        f"{self.name}\{subgroup.name}:{repr.name}",
                        irr.name,
                        m,
                        n_repetitions,
                    )
                    ...
                    for shift in range(m):
                        ...
                        v[:, p : p + irr.size] = (
                            v[:, p : p + irr.size] @ restricted_irr.change_of_basis_inv
                        )
            ...
            raise ValueError(
                "Induction from discrete subgroups of O(2) leads to infinite"
                " dimensional induced representations. Hence, induction from the"
                f" subgroup identified by {subgroup_id} is not allowed."
            )

    @staticmethod
    def _generator(maximum_frequency = 10):
