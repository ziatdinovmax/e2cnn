...
class DihedralGroup(Group):
    ...
    def inverse(self, element):
        ...
        return (
            element[0],
            (-element[1] * (-1 if element[0] else 1)) % self.rotation_order,
        )
    ...
    def combine(self, e1, e2):
        ...
        return (
            (e1[0] + e2[0]) % 2,
            (e1[1] + (-1 if e1[0] else 1) * e2[1]) % self.rotation_order,
        )
    ...
    def subgroup(self, id):
        ...
        assert self.rotation_order % order == 0, (
            "Error! The rotations order of the subgroups of a dihedral group has to"
            " divide the rotations order of the overgroup. %d does not divide %d "
        ...
        if id not in self._subgroups:
            ...
            if id[0] is not None and id[1] == 1:
                ...
                child_mapping = (
                    lambda e, axis=axis: None if e[1] != e[0] * axis else e[0]
                )
                ...
                child_mapping = (
                    lambda e, ratio=ratio: None
                    if (e[0] != 0 or e[1] % ratio > 0)
                    else int(e[1] / ratio)
                )
                ...
                parent_mapping = lambda e, ratio=ratio, axis=axis: (
                    e[0],
                    e[1] * ratio + e[0] * axis,
                )
                child_mapping = (
                    lambda e, ratio=ratio, axis=axis: None
                    if (e[1] - e[0] * axis) % ratio > 0
                    else (e[0], int((e[1] - e[0] * axis) / ratio))
                )
        ...
        if name not in self.irreps:
            ...
            if j == 0:
                ...
                if k == 0:
                    ...
                    irrep = lambda element, k=k, base_angle=base_angle: np.array(
                        [[np.cos(k * element[1] * base_angle)]]
                    )
                ...
                if k == 0:
                    ...
                    irrep = lambda element, k=k, base_angle=base_angle: utils.psichi(
                        element[1] * base_angle, element[0], k=k
                    )

                    # build the trace of this matrix
                    character = (
                        lambda element, k=k: 0
                        if element[0]
                        else 2 * np.cos(k * element[1])
                    )
