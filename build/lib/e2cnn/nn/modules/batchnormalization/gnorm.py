...
class GNormBatchNorm(EquivariantModule):
    def __init__(
        self, in_type, eps = 1e-05, momentum = 0.1, affine = True,
    ):
        r"""
        ...
        for r in self.in_type._unique_representations:
            ...
            for i, irr in enumerate(r.irreps):
                ...
                aggregator[p : p + irr.size, i] = 1.0 / irr.size
            ...
            if not np.allclose(r.change_of_basis, np.eye(r.size)):
                self.register_buffer(
                    f"{name}_change_of_basis",
                    torch.tensor(r.change_of_basis, dtype=torch.float),
                )
                self.register_buffer(
                    f"{name}_change_of_basis_inv",
                    torch.tensor(r.change_of_basis_inv, dtype=torch.float),
                )
            ...
            running_var = torch.ones(
                (1, self._nfields[r.name], len(r.irreps), 1, 1), dtype=torch.float
            )
            running_mean = torch.zeros(
                (1, self._nfields[r.name], len(trivials), 1, 1), dtype=torch.float
            )
            ...
            if self.affine:
                weight = Parameter(
                    torch.ones((1, self._nfields[r.name], len(r.irreps), 1, 1)),
                    requires_grad=True,
                )
                bias = Parameter(
                    torch.zeros((1, self._nfields[r.name], len(trivials), 1, 1)),
                    requires_grad=True,
                )
    ...
    def _compute_statistics(self, t, name):
        ...
        trivial_means = (
            t[:, :, trivial_idxs, ...]
            .view(b, c, l, x, y)
            .sum(dim=(0, 3, 4), keepdim=True)
            .detach()
            / N
        )

        # compute the mean of squares of all channels
        vars = (t ** 2).view(b, c, s, x, y).sum(
            dim=(0, 3, 4), keepdim=True
        ).detach() / N
    ...
    def _scale(self, t, scales, name, out = None):
    ...
    def _shift(self, t, trivial_bias, name, out = None):
