...
class InducedGatedNonLinearity1(EquivariantModule):
    def __init__(self, in_type, gates = None, drop_gates = True, **kwargs):
        r"""
        ...
        for g, r in zip(gates, in_type.representations):
            if g == GATES_ID:
                ...
                for nl in r.supported_nonlinearities:
                    ...
                    raise ValueError(
                        'Error! Representation "{}" does not support "gated"'
        ...
        assert ngates == ngated, (
            "Error! Number of gates ({}) does not match the number of gated"
            " non-linearities required ({})".format(ngates, ngated)
        )
        ...
        self.bias = Parameter(
            torch.randn(1, ngates, 1, 1, 1, dtype=torch.float), requires_grad=True
        )
    ...
    def forward(self, input):
        ...
        output = torch.empty(
            b, self.out_type.size, h, w, dtype=torch.float, device=self.bias.device
        )
        ...
        for type in self._order:
            ...
            if self._contiguous[type]:
                ...
                output[:, output_indices, :, :] = (
                    input[:, input_indices, ...].view(b, -1, subfield_size, h, w) * g
                ).view(b, -1, h, w)
