...
class GatedNonLinearity2(EquivariantModule):
    def __init__(self, in_type, **kwargs):
        ...
        assert len(in_type[0]) == len(in_type[1]), (
            ...
            " non-linearities required ({})".format(len(in_type[0]), len(in_type[1]))
        )
        ...
        self.bias = Parameter(
            torch.randn(1, ngates, 1, 1, dtype=torch.float), requires_grad=True
        )

    def forward(self, gates, input):
        r"""
        ...
        for size in self._order:
            ...
            if self._contiguous[size]:
                ...
                output[:, indices[0] : indices[1], ...] = (
                    input[:, indices[0] : indices[1], ...].view(b, -1, size, h, w) * g
                ).view(b, -1, h, w)

            else:
                # otherwise we have to use indexing
                output[:, indices, :, :] = (
                    input[:, indices, ...].view(b, -1, size, h, w) * g
                ).view(b, -1, h, w)
    ...
    def evaluate_output_shape(self, *input_shapes):
