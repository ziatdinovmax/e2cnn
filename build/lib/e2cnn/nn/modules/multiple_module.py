...
class MultipleModule(EquivariantModule):
    def __init__(self, in_type, labels, modules, reshuffle = 0):
        r"""
        ...
        assert (modules_labels in all_labels) or (modules_labels == all_labels), (
            "Error! Some labels assigned to the modules don't appear among the channels"
            " labels"
        )
        ...
        for module, l in modules:
            if isinstance(l, str):
                assert module.in_type == self.branching.out_type[l], (
                    f"Label {l}, branch class and module ({module}) class don't"
                    f" match:\n [{module.in_type}] \n [{self.branching.out_type[l]}]\n"
                )
            else:
                for i, lb in enumerate(l):
                    assert module.in_type[i] == self.branching.out_type[lb], (
                        f"Label {lb}, branch class and module ({module}) class [{i}]"
                        f" don't match: \n [{module.in_type[i]}] \n"
                        f" [{self.branching.out_type[lb]}]\n"
                    )
    ...
    def check_equivariance(self, atol = 2e-6, rtol = 1e-5, full_space_action = True):
