...
class GeometricTensor:
    def __init__(self, tensor, type):
        ...
        *transformation law*.
        ...
        assert tensor.shape[1] == type.size, (
            f"Error! The size of the tensor {tensor.shape} does not match the size of"
            f" the field type {type.size}."
        )
    ...
    def transform_fibers(self, element):
        r"""
        ...
        data = torch.einsum(
            "oi,bihw->bohw", (rho, self.tensor.contiguous())
        ).contiguous()
    ...
    def __add__(self, other):
        ...
        ), "The two geometric tensor must have the same FieldType"
    ...
    def __mul__(self, other):
        ...
        """
        assert (
            isinstance(other, float) or other.numel() == 1
